...
namespace SeatAllocationWebApi.Repository
{
    ...
    public class FloorStructureRepository: IFloorStructureRepository
    {
        ...
        {
            return _context.FloorStructures.ToList();
        }

        public IEnumerable<FloorStructure> GetByBuildingCode(string id)
        {
            return _context.FloorStructures.ToList().Where(l => l.BuildingCode == id);
        }

        public FloorStructure Getid(int id)
        {
            //Select floor to be updated by floorId and returning it to the client
            return _context.FloorStructures.FirstOrDefault(m => m.FloorCode == id);          
        }
